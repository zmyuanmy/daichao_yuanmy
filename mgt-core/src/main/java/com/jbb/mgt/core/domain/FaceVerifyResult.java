...
public class FaceVerifyResult {
    ...
    // "PASSED"：流程验证通过。 "VIDEO_SR_ERROR"：语音识别结果与要求不符。（仅当视频验证流程时）
    // "VIDEO_NOT_SYNCHRONIZED"：视频中唇语识别错误。（仅当视频验证流程时）"VIDEO_FACE_INCONSISTENT":
    // 视频过程中的人脸不一致
    private String procedureNalidation;
    // 假脸攻击判定的结果，它取如下值： “PASSED”：不是假脸。 “MASK”：面具攻击。 “SCREEN_REPLAY”：屏幕翻拍。
    ...
    public String toString() {
        return "FaceVerifyResult [requestId=" + requestId + ", timeUsed=" + timeUsed + ", bizNo=" + bizNo
            ...
            + ", error_message=" + error_message + ", sim=" + sim + ", flagVerify=" + flagVerify + ", verifyData="
            + verifyData + "]";
    }
