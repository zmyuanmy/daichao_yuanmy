...
-- 点击PV
INSERT INTO  mgt_channel_pv (statistic_date,channel_code,h5_type,is_delegate,pv,uv) 
select 
	...
	sum(l2.pv) as pv,
	count(distinct l1.cookie_id) as uv
from  mgt_user_event_logs l1
join  
(
	 select log.cookie_id, max(log.creation_date) as creation_date count(1)as pv from mgt_user_event_logs log
