...
@Service("FinOrgRegisterStatisticServices")
public class FinOrgRegisterStatisticServicesImpl implements FinOrgStatisticServices {
    // 注册为2
            ...
            if (organizationIncludeSales != null && organizationIncludeSales.size() != 0) {
                for (OrganizationIncludeSales organizationIncludeSales1 : organizationIncludeSales) {
                    ...
                    if (userCounts != null && userCounts.size() != 0) {
                        cnt = userCounts.get(0).getRegCnt();
                    }
                    mgtFinOrgStatisticDaily.setCnt(cnt);
                    int defaultPrice = organizationIncludeSales1.getDefaultRegisterPrice();
