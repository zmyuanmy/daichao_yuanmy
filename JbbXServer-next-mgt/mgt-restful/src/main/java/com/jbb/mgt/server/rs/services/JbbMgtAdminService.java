...
import com.jbb.mgt.rs.action.dunxing.DunXingAction;
...
public class JbbMgtAdminService extends BasicRestfulServices {
    ...
    public String notifyDunXing(@Context HttpServletRequest request) throws IOException {
        logger.debug(">notifyDunXing()");
        DunXingAction action = getBean(DunXingAction.ACTION_NAME);
        action.notifyMsgCode(request);
        logger.debug("<notifyDunXing()");
        return "success";
    }

    @GET
    @Path("/human/phone/{phoneNumber}")
    public ActionResponse getHumanLpPhone(@HeaderParam(API_KEY) String userKey,
        @PathParam("phoneNumber") String phoneNumber, @QueryParam("startDate") String startDate,
        @QueryParam("endDate") String endDate) {
        logger.debug(">getHumanLpPhone()");
        DunXingAction action = getBean(DunXingAction.ACTION_NAME);
        action.validateUserKey(userKey);
        action.validateJbbAccount();
        action.selectHumanLpPhone(phoneNumber, startDate, endDate);
        logger.debug("<getHumanLpPhone()");
        return action.getActionResponse();
    }

    @POST
    @Path("/dunxing/sendMsg")
    public ActionResponse sendDunXingMsg(@HeaderParam(API_KEY) String userKey,
        @QueryParam("phoneNumber") String phoneNumber, @QueryParam("inviteCode") String inviteCode,
        @QueryParam("date") String date, @QueryParam("msgModeId") String msgModeId) {
        logger.debug(">sendDunXingMsg()");
        DunXingAction action = getBean(DunXingAction.ACTION_NAME);
        action.validateUserKey(userKey);
        action.validateJbbAccount();
        action.sendMsg(inviteCode, phoneNumber, date, msgModeId);
        logger.debug("<sendDunXingMsg()");
        return action.getActionResponse();
    }
    ...
    @GET
    @Path("/dunxing/msgLog")
    public ActionResponse getMsgLog(@HeaderParam(API_KEY) String userKey, @QueryParam("date") String date,
        @QueryParam("status") String status, @QueryParam("phoneNumber") String phoneNumber) {
        logger.debug(">getMsgLog()");
        DunXingAction action = getBean(DunXingAction.ACTION_NAME);
        action.validateUserKey(userKey);
        action.validateJbbAccount();
        action.selectMsgLog(date, status, phoneNumber);
        logger.debug("<getMsgLog()");
        return action.getActionResponse();
    }
    ...
    @Path("/dunxing/notify")
    @Consumes({MediaType.APPLICATION_FORM_URLENCODED})
    public String notifyDunXingNew(@Context HttpServletRequest request) throws IOException {
        logger.debug(">notifyDunXingNew()");
        DunXingAction action = getBean(DunXingAction.ACTION_NAME);
        action.notifyMsgCodeNew(request);
        logger.debug("<notifyDunXingNew()");
        return "success";
    }
    ...
    @POST
    @Path("/miaodi/notify")
    public void notifyMiaoDiMsg(@Context HttpServletRequest request, @Context HttpServletResponse response)
        throws IOException {
        logger.debug(">notifyMiaoDiMsg()");
        DunXingAction action = getBean(DunXingAction.ACTION_NAME);
        action.notifyMiaoDiMsg(request);
        String respCode = "{\"respCode\":\"00000\"}";
        response.getWriter().write(respCode);
