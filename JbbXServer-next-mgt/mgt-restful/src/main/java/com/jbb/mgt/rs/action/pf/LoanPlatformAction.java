...
public class LoanPlatformAction extends BasicAction {
    ...
    public void getPlatformByTagId(int[] areaTagId, Boolean getConfig, String os, boolean isOld) {
        ...
        // 检查 referer , 借帮帮 线上android 显示合规产品, ios 显示正规产品
        boolean refFlag = false;
        String ref = this.getHttpRequest().getHeader("Referer");
        refFlag = ipAddressService.checkReferer(ref);
//        if (isOld) {
//            String ref = this.getHttpRequest().getHeader("Referer");
//            refFlag = ipAddressService.checkReferer(ref);
//
//            if (!refFlag) {
                // 如果是旧的android 借帮帮，获取合规产品
//                List<LoanTag> loanTags = loanPlatformService.getLoanTags(true);
//                List<Integer> areaTagIdList = new ArrayList<Integer>();
//                for (LoanTag loanTag : loanTags) {
//                    // 合规产品的areaId
//                    int areaId = loanTag.getAreaId();
//                    if(!CommonUtil.inArray(areaId, areaTagId)){
//                        continue;
//                    }
//                    if (areaId >= 800 && areaId < 900) {
//                        areaTagIdList.add(loanTag.getAreaTagId());
//                    }
//                }
//                areaTagId = new int[areaTagIdList.size()];
//                for (int i = 0; i < areaTagIdList.size(); i++) {
//                    areaTagId[i] = areaTagIdList.get(i);
//                }
//            }
//
//        }
        ...
        // 合规产品处理 - 硬编码
//        if ((isOld && !refFlag) || ipAddressService.isIpShowValidProduct(this.getRemoteAddress())) {
        if (ipAddressService.isIpShowValidProduct(this.getRemoteAddress())) {

            for (AreaPlatformVo apf : apfs) {
                // 合规产品的areaId
                int areaId = apf.getAreaId();
                if (areaId >= 800 && areaId < 900) {
                    apf.setAreaId(areaId - 800);
                    // areaId是固定的6 表示首页强推 如果是随机获取一个
                    if (apf.getAreaId().equals(6)) {
                        List<Platform> list = new ArrayList<Platform>();
                        if (loanPlatformService.getPlatformHomePagePush(os) != null) {
                            list.add(loanPlatformService.getPlatformHomePagePush(os));
                        }
                        apf.setPlatforms(list);
                    }
                }
            }
        }
