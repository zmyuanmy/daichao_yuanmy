...
public class H5MerchantAction extends BasicAction {
    ...
    public void selectH5ConfigByChannelCode(String channelCode, String platform) {
        ...
        try {
            ...
            if(null !=channel){
                H5Merchant h5Merchant = null;
                if("ios".equals(platform) && channel.getMerchantId2() != null ){
                    h5Merchant = service.selectH5merchantById(channel.getMerchantId2());
                }else if(channel.getMerchantId() != null){
                    h5Merchant = service.selectH5merchantById(channel.getMerchantId());
                }
                
                if (null != h5Merchant) {
                    this.response.h5 = h5Merchant;
                }
                
                this.response.testFlag = channel.isTestFlag();
                AppInfo appInfo = new AppInfo();
                this.response.downloadAppInfo  = appInfo;
                String appId = channel.getDownloadApp();
                appInfo.setAppId(appId);
                appInfo.setAppId(PropertyManager.getProperty("jbb.h5.download.app."+appId+".id"));
                appInfo.setDefaultUrl(PropertyManager.getProperty("jbb.h5.download.app."+appId+".url.default"));
                appInfo.setIosUrl(PropertyManager.getProperty("jbb.h5.download.app."+appId+".url.ios"));
                appInfo.setAndroidUrl(PropertyManager.getProperty("jbb.h5.download.app."+appId+".url.android"));
                appInfo.setXiaomiUrl(PropertyManager.getProperty("jbb.h5.download.app."+appId+".url.xiaomi"));
                appInfo.setHuaweiUrl(PropertyManager.getProperty("jbb.h5.download.app."+appId+".url.huawei"));
                appInfo.setVivoUrl(PropertyManager.getProperty("jbb.h5.download.app."+appId+".url.vivo"));
                appInfo.setOppoUrl(PropertyManager.getProperty("jbb.h5.download.app."+appId+".url.oppo"));
            }else{
